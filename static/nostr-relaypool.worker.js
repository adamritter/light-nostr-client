    ...
    case "nevent": {
      ...
      return {
        ...
        data: {
          ...
          relays: tlv[1] ? tlv[1].map((d) => utf8Decoder.decode(d)) : [],
          author: ((_c = tlv[2]) == null ? void 0 : _c[0]) ? utils.bytesToHex(tlv[2][0]) : void 0
        }
      };
    }
    case "naddr": {
      let tlv = parseTLV(data);
      if (!((_d = tlv[0]) == null ? void 0 : _d[0]))
        throw new Error("missing TLV 0 for naddr");
      if (!((_e = tlv[2]) == null ? void 0 : _e[0]))
        throw new Error("missing TLV 2 for naddr");
      if (tlv[2][0].length !== 32)
        throw new Error("TLV 2 should be 32 bytes");
      if (!((_f = tlv[3]) == null ? void 0 : _f[0]))
...
async function queryProfile(fullname) {
  ...
  if (!((_a = res == null ? void 0 : res.names) == null ? void 0 : _a[name]))
    return null;
  let pubkey = res.names[name];
  let relays = ((_b = res.relays) == null ? void 0 : _b[pubkey]) || [];
  return {
    pubkey,
    relays
  };
...
var RelayC = class {
  ...
  async connectRelay() {
    return new Promise((resolve, reject) => {
      ...
      this.ws = ws2;
      ws2.onopen = __privateMethod(this, _onopen, onopen_fn).bind(this, resolve);
      ws2.onerror = (e) => {
        this.listeners.error.forEach((cb) => cb());
        reject(e);
      };
      ws2.onclose = __privateMethod(this, _onclose, onclose_fn).bind(this);
      ws2.onmessage = __privateMethod(this, _onmessage, onmessage_fn).bind(this);
    });
  }
  async connect() {
    var _a;
    if (((_a = this.ws) == null ? void 0 : _a.readyState) && this.ws.readyState === 1)
      return;
    try {
...
var EventCache = class {
  ...
  getCachedEventsWithUpdatedFilters(filters, relays) {
    ...
    for (const filter of filters) {
      const new_data = __privateMethod(this, _getCachedEventsByIdWithUpdatedFilter, getCachedEventsByIdWithUpdatedFilter_fn).call(this, filter) || __privateMethod(this, _getCachedEventsByPubKeyWithUpdatedFilter2, getCachedEventsByPubKeyWithUpdatedFilter2_fn).call(this, filter) || __privateMethod(this, _getCachedEventsByTagsWithUpdatedFilter, getCachedEventsByTagsWithUpdatedFilter_fn).call(this, filter) || {
        filter,
        events: []
      };
...
var _subscribeRelay, subscribeRelay_fn, _mergeAndRemoveEmptyFiltersByRelay, mergeAndRemoveEmptyFiltersByRelay_fn, _subscribeRelays, subscribeRelays_fn, _resetTimer, resetTimer_fn, _getRelaysAndSubscribe, getRelaysAndSubscribe_fn;
var RelayPool = class {
  constructor(relays, options = {}) {
    ...
    __publicField(this, "startTime", new Date().getTime());
  ...
  for (const [relay, filters] of filtersByRelay) {
    ...
    const subOnEose = (url, minCreatedAt) => {
      ...
      if (unsuboneosecbcalled) {
        (_a = subHolder.sub) == null ? void 0 : _a.unsub();
      } else {
        if (subHolder.sub) {
